 ...
 */
// Avoid direct require() of @whiskeysockets/baileys (ESM). Provide local helpers
// Lightweight replacement for getContentType (sufficient for our usage here)
const getContentType = (msg) => {
    if (!msg) return null
    const types = ['conversation','imageMessage','videoMessage','extendedTextMessage','buttonsResponseMessage','listResponseMessage','documentMessage','stickerMessage','audioMessage','viewOnceMessage','productMessage','ephemeralMessage']
    for (const t of types) if (msg[t]) return t
...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            // Create a lightweight fake message object instead of using proto.WebMessageInfo
